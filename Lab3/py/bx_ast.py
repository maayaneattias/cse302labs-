...
class Expr(Node):
    # TO BE EXTENDED true, false
    # I don't think we need to extend the load since we don't load now we have lexer and parser, no?
...
class Boolean(Expr):
    ...
    @property
    def js_obj(self):
        return {'tag': 'Boolean', 'value': str(self.value)}
...
class Stmt(Node):
    # TO BE EXTENDED block, ifelse, while, jump
    # I don't think we need to extend the load since we don't load now we have lexer and parser, no?
...
class Ifrest(Stmt):
    ...
    @property
    def js_obj(self):
        # CHANGE if needed - Guru's question
        return {'tag': 'Print', 'arg': self.arg.js_obj}


class Ifelse(Stmt):
    # TO DO
    #init: (sloc, Expr , Block ,Ifrest )
    def __init__(self, sloc, expr: Expr, blc: Block, rest: Ifrest):
        """
...
class While(Stmt):
    #init: (sloc, Expr, Block)
    # Stack needed because of the jumps (at least in my opinion)
    pass


class Break(Stmt):
    #init: (sloc)
    # Agree on stack manipulation for this
    pass
